...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--shazam_coverart", type=str, default="", help="save shazam cover art too"
    )
    parser.add_argument(
        "--netease",
        action="store_true",
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler("./inaseg_inst.log"), logging.StreamHandler()],
    )
    ...
    if (
        len(
            glob.glob(
                os.path.join(
                    args.outdir,
                    f"*{os.path.splitext(os.path.basename(media))[0][1:]}_*",
                )
            )
        )
        == 0
        ...
        try:
            ...
            saved_timestamp = extract_music(
                segment_wrapper(media, segment_length_thres=args.max_segment_length),
