...
def get_length_using_copied_audio(filename: str):
    temp_audio_file = os.path.join(tempfile.gettempdir(), "get_length_acodec_temp.mp4")
    try:
...
def get_segment_process_length_array(filename: str, thres: int = 0):
    ...
    if file_length == 0:
        logging.warning(
            f"ffprobe on {filename} length failed.\
    ...
    logging.debug(
        (
            f"filelength {str(file_length)} is \
        larger than thres {str(thres)}, triggering a segmentation."
        )
    )
    result = [
        [x * thres, (x + 1) * thres] for x in range(math.ceil(file_length / thres))
    ]
    result[0][0] = None
    result[-1][1] = None
    return result
